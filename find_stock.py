...
def valid_stock(dates):
    ...
    for i in range(0, len(value_code)):
        if re.match('000', value_code[i][0]) or re.match('002', value_code[i][0]):
            ...
            # try:
            cursor.execute('select * from stock_' + value_code[i][0] + ' order by date desc limit 6')  # 当天
            ...
            # 收盘价
            p_change0 = float(value[0][6])
            # close1 = float(value[1][2])
            # close2 = float(value[2][2])
            # close3 = float(value[3][2])
            # close4 = float(value[4][2])
            # close5 = float(value[5][2])
            # 涨幅
            # p_change0 = float(value[0][6])
            # p_change1 = float(value[1][6])
            # p_change2 = float(value[2][6])
            # p_change3 = float(value[3][6])
            # p_change4 = float(value[4][6])
            # p_change5 = float(value[5][6])
            # 下跌股票数量
            count_change_down = 0
            count_change_2 = 0
            p_change_max = p_change0
            for e in value:
                change = float(e[6])
                if change < 0.0:
                    count_change_down += 1
                if change >= -2.0 and change <= 2.0:
                    count_change_2 += 1
                if p_change_max < change:
                    p_change_max = change
            if count_change_down > 3 and count_change_down < 6 and p_change_max > 9.6 and count_change_2 >= 3:
                count.append(value_code[i][0])
                a += 1
    # except:
